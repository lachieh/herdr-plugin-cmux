...
const MARKER_RE = /hpcx:([0-9a-f]{12})/;
...
export function describeRow(action) {
  return [`${action.agent || 'herdr'} agent`, tidyPath(action.cwd), marker(action.key)]
    .filter(Boolean)
    .join(' · ');
}
  ...
  switch (action.type) {
    case 'create': {
      const args = [
        'new-workspace',
        '--name', action.label,
        '--description', describeRow(action),
        '--focus', 'false',
      ];
      if (action.cwd) args.push('--cwd', action.cwd);
      if (cfg.window) args.push('--window', cfg.window);
      if (cfg.clickMode === 'attach' && action.terminalId) {
        args.push('--command', `exec herdr agent attach ${action.terminalId}`);
      } else {
