...
const tidyPath = (p) => (HOME && p && p.startsWith(HOME) ? `~${p.slice(HOME.length)}` : p || '');

/**
 * Human-first row description: "claude agent · ~/Projects/x · hpcx:1a2b3c4d5e6f".
 * With `withTerm` (attach mode) the marker records the seeded terminal so a later
 * terminal change is detectable: "… · hpcx:1a2b3c4d5e6f+term_abc".
 */
export function describeRow(action, withTerm = false) {
  const tag =
    withTerm && action.terminalId ? `${marker(action.key)}+${action.terminalId}` : marker(action.key);
  return [`${action.agent || 'herdr'} agent`, tidyPath(action.cwd), tag]
    .filter(Boolean)
    .join(' · ');
}
  ...
  switch (action.type) {
    case 'create': {
      ...
      const args = [
        'new-workspace',
        '--name', action.label,
        '--description', describeRow(action, attach),
        '--focus', 'false',
      ];
    ...
    case 'reseed': {
      ...
      // Group membership and the task line belong to the old workspace — clear the
      // caches so the follow-up passes re-apply them to the new one.
