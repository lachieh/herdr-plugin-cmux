...
export function reconcile(roster, mirrors = [], cfg = {}) {
  ...
  for (const a of roster) {
    ...
    // Attach rows record the terminal they were seeded with; if the agent's
    // terminal moved (pane compaction, herdr daemon restart), the hosted attach
    // is stale or dead — replace the row. Only in attach mode, only when the
    // mirror actually carries a seeded terminal (legacy/focus rows never reseed).
    if (
