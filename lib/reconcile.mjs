...
//   Mirror    = { key, wsUuid, status, label? }          // a cmux workspace we created
//   Action    =
//       | { type: 'create', key, label, cwd, terminalId, status }
//       | { type: 'status', key, wsUuid, label, status }
//       | { type: 'close',  key, wsUuid }
//       | { type: 'notify', key, wsUuid?, label, status }
//
// Config:
//   { removeOnExit=true, holdUnknown=true, notifyOn=Set|[], suppress=Set|[] }
